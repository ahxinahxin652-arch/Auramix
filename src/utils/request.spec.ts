  ...
  describe('response interceptor', () => {
    it('returns data on code 200', async () => {
      service.defaults.adapter = vi
        .fn()
        .mockResolvedValue(mockReply({ code: 200, data: { foo: 1 }, message: 'ok' }))
    ...
    it('clears auth, warns and redirects on 4031', async () => {
      ...
      service.defaults.adapter = vi
        .fn()
        .mockResolvedValue(mockReply({ code: 4031, data: null, message: '账号已被停用' }))
    ...
    it('rewrites 5000 message to 服务异常', async () => {
      ...
      service.defaults.adapter = vi
        .fn()
        .mockResolvedValue(mockReply({ code: 5000, data: null, message: 'SQL exception xyz' }))
    ...
    it('passes through 400/4011 message', async () => {
      ...
      service.defaults.adapter = vi
        .fn()
        .mockResolvedValue(mockReply({ code: 4011, data: null, message: '用户名或密码错误' }))
