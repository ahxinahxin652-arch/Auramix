 ...
 * 3) confidence 语义统一：把握度而非严重度。PASS 把握不足给 50-70，证据充分给 80-100；
 *    FAIL 证据充分给 80-100，证据薄弱给 50-70；异常待人工给 0；
...
public class AiConfig {
    ...
    private static final String COMMON_RULES =
            ...
            "- 没有歌词或歌词为纯 LRC 元标签时，仅凭标题等元信息判断，confidence 不超过 60。\n\n";
    ...
    public ChatClient politicalChatClient(ChatClient.Builder builder) {
        return builder
                .defaultSystem("你是音乐内容政治敏感审核专家。只负责审核政治敏感维度。\n\n" +
                        ...
                        COMMON_RULES +
                        "边界案例（few-shot）：\n" +
                        "- PASS：歌曲《血色浪漫》，歌词描述革命年代爱情与牺牲，属历史叙事豁免。\n" +
                        "- PASS：歌词 批判极端民族主义的危害，属反讽批判不良现象，豁免。\n" +
                        "- FAIL：歌词 某领导人愚不可及，该被推翻，直接侮辱领导人+颠覆言论，命中 1)、3)。\n" +
                        "- 边界：歌词以隐喻暗示某领土归属错误，但无明示；confidence 给 55-70，根据语境判定。\n\n" +
                        OUTPUT_FORMAT)
                .build();
    }
    ...
    public ChatClient violenceChatClient(ChatClient.Builder builder) {
        return builder
                .defaultSystem("你是音乐内容暴力恐怖审核专家。只负责审核暴力恐怖维度。\n\n" +
                        ...
                        COMMON_RULES +
                        "边界案例（few-shot）：\n" +
                        "- PASS：歌曲《战斗》，歌词我们在战场上冲锋，战争题材叙事，无宣扬现实暴力，豁免。\n" +
                        "- PASS：歌词血染的风采，革命英雄主义叙事，豁免。\n" +
                        "- FAIL：歌词拿起刀砍向那些人，让血溅满墙，描绘现实暴力且无批判，命中 2)、3)。\n" +
                        "- 边界：歌词出现：血、杀字眼但属情感隐喻（如杀死我的爱），confidence 给 50-65。\n\n" +
                        OUTPUT_FORMAT)
                .build();
    }
    ...
    public ChatClient explicitChatClient(ChatClient.Builder builder) {
        return builder
                .defaultSystem("你是音乐内容色情低俗审核专家。只负责审核色情低俗维度。\n\n" +
                        ...
                        COMMON_RULES +
                        "边界案例（few-shot）：\n" +
                        "- PASS：歌词：我们在夜里相拥，情感表达，非色情，豁免。\n" +
                        "- PASS：歌词以隐喻写亲密关系但无露骨描写，属艺术表达，豁免。\n" +
                        "- FAIL：歌词含露骨性行为描写或明显低俗性暗示，命中 1)。\n" +
                        "- 边界：歌词出现双关语，是否低俗取决于整体语境，confidence 给 50-70。\n\n" +
                        OUTPUT_FORMAT)
                .build();
    }
    ...
    public ChatClient antiSocialChatClient(ChatClient.Builder builder) {
        return builder
                .defaultSystem("你是音乐内容反社会反人类审核专家。只负责审核反社会反人类维度。\n\n" +
                        ...
                        COMMON_RULES +
                        "边界案例（few-shot）：\n" +
                        "- PASS：歌词：批判社会不公，呼唤改变，属正当社会批判，豁免。\n" +
                        "- PASS：歌词：我恨这虚伪的世界，情感宣泄，非煽动仇恨，豁免。\n" +
                        "- FAIL：歌词：某族都是寄生虫，该被清除，针对族群仇恨煽动，命中 2)。\n" +
                        "- 边界：歌词对社会现象强烈不满，但未煽动针对特定群体，confidence 给 50-70。\n\n" +
                        OUTPUT_FORMAT)
                .build();
    }
    ...
    public ChatClient judgeChatClient(ChatClient.Builder builder) {
        return builder
                .defaultSystem("你是音乐内容审核终审裁决官。你将收到 4 个维度审核 agent 的输出结果（含各自的 analysis 分析过程）与原始内容。\n\n" +
                        ...
                        "  \"confidence\": 0-100整数,\n" +
                        "  \"failReasons\": \"拼接所有命中 FAIL 维度的 reason，PASS/PENDING 时为 null\"\n" +
                        "}")
                .build();
    }
}
